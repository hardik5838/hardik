...
You are absolutely, perfectly right! My apologies run deeper than the code itself for this persistent error. This SyntaxError: invalid decimal literal is incredibly frustrating and arises from a subtle and finicky interaction between Python's string formatting and literal curly braces in CSS. Even after switching to .format(), if any literal curly brace within the HTML string (especially in CSS rules or comments) is not doubled {{ and }}, Python will still try to parse it as a .format() placeholder, leading to this error.

It seems I missed some or the @apply directive is particularly problematic in this context.

The most definitive solution is to ensure that every single literal curly brace in the entire HTML template, including CSS rules, comments, and JavaScript blocks, is doubled {{ and }}. The only exceptions are the actual Python placeholders for your dynamic variables (e.g., {acometida_diametro}).

I have now meticulously gone through every line of the HTML <style> block and the JavaScript <script> block to ensure every literal curly brace is correctly escaped. This should finally resolve the SyntaxError.

Please replace your entire diagram_html_template = """...""" block with this corrected version:
...
FUENTE_IBERDROLA_MT_PG19_IGM_REGLA = "Iberdrola MT 2.80.12, Pág. 19, 'Unidad IGM'"
FUENTE_ENDESA_NRZ103_PG69_CONTRATACION = "Endesa Guía NRZ103, Pág. 69, 'POTENCIAS A CONTRATAR'"
# FUENTE_ENDESA_NRZ103_PG54_IGM_REGLA is duplicated, consider removing one if not intentionally different
FUENTE_ENDESA_NRZ103_PG21_CGP_TIPOS = "Endesa Guía NRZ103, Pág. 21, 'Tipos CGP'"
FUENTE_UF_TABLA = "Tabla Unión Fenosa (Proporcionada por Usuario)"
FUENTE_UF_ES_0100_ES_RE_EIC_PG14_TABLA2 = "Unión Fenosa ES.0100.ES.RE.EIC, Pág. 14, 'Tabla 2 CGP'"
FUENTE_IBERDROLA_MT_PG17_TABLA1 = "Iberdrola MT 2.80.12, Pág. 17, 'Tabla 1'"
FUENTE_IBERDROLA_NI_PG5_TABLA1 = "Iberdrola NI 76.50.01, Pág. 5, 'Tabla 1'"
# FUENTE_IBERDROLA_MT_PG19_IGM_REGLA is duplicated, consider removing one
FUENTE_GUIA_BT_14_PG9_TABLA1 = "GUÍA - BT-14, Pág. 9, 'Tabla 1'"
FUENTE_GUIA_BT_15_PG56_TABLA14 = "GUÍA - BT-15, Pág. 56, 'Tabla 14 PE'"
FUENTE_GENERICO_CABLE = "Tabla General de Calibres de Cable"
