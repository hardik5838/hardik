...
# Nota: Los valores de 'power_kw' en esta tabla son puntos de referencia proporcionados por el usuario
# que representan umbrales de potencia para las secciones de cable y longitudes máximas específicas de Unión Fenosa.
# No son necesariamente derivados de un documento oficial completo de UF en esta versión, sino como base de datos de usuario.
...
# FUENTE_UF_ES_0100_ES_RE_EIC_PG14_TABLA3 = "Unión Fenosa ES.0100.ES.RE.EIC, Pág. 14, 'Tabla 3 CP/BTVC'" # No utilizada directamente, pero es la fuente de CP/BTVC
...
# Aquí interpretamos 'nominal_protection_current_a' como la Intensidad Nominal (A) de protección/fusible
endesa_contracted_power_data = [
    {"power_kw": {"valor": 3.46, "fuente": FUENTE_ENDESA_NRZ103_PG69_CONTRATACION}, "base_contractual_a": {"valor": 5, "fuente": FUENTE_ENDESA_NRZ103_PG69_CONTRATACION}, "nominal_protection_current_a": {"valor": 40, "fuente": FUENTE_ENDESA_NRZ103_PG69_CONTRATACION}}, # CORRECTED VALUE HERE for 3.46kW
...
# Tabla 2: Tipos de CGP y valores máximos de potencia disipada en fusibles
uf_cgp_fuse_data = [
...
def get_guia_bt_15_ground_size_by_phase(phase_mm2_ref):
    ...
    if phase_mm2_ref <= 16:
        ...
        return max(16, math.ceil(phase_mm2_ref / 2))
...
st.image("Logo_ASEPEYO.png", width=150) # Puedes ajustar el 'width' para cambiar el tamaño del logo
...
st.info("También puede introducir directamente la corriente de diseño si la conoce (anula el cálculo de potencia para fines de dimensionamiento inicial).")
input_design_current_a = st.number_input(
    "Corriente de Diseño Calculada (A) (Opcional)",
    min_value=0.0, value=0.0, step=1.0,
    help="Si se proporciona y es mayor que 0, esta corriente se utilizará directamente para fines de dimensionamiento. Si la potencia contratada es 0 kW y la corriente se proporciona, los resultados se basarán solo en esta corriente y en las tablas genéricas."
)
...
# --- Lógica principal de selección de datos y advertencias ---
# Si la potencia es 0 kW Y la corriente de diseño es > 0, NO usamos las tablas basadas en kW de la compañía
use_company_power_tables = True
if power_kw == 0.0 and input_design_current_a > 0:
    use_company_power_tables = False
    st.warning("Potencia contratada (0 kW) no válida para la búsqueda en tablas específicas de la compañía. Los resultados se basarán en la corriente de diseño proporcionada y en tablas genéricas o reglas generales.")
    st.info("*(Para obtener resultados específicos de la compañía, proporcione una Potencia Máxima Contratada mayor que 0 kW.)*")
...
if use_company_power_tables:
    ...
    pass # No company-specific data lookup if power_kw is 0
...
if selected_company_data and use_company_power_tables: # Only show company-specific data if power_kw was used for lookup
    ...
    if company == "Endesa":
        ...
        # NUEVO: Advertencia específica para CGP-1-100/BUC de Iberdrola
        if cgp_type == "CGP-1-100/BUC":
            st.warning(f"*(Advertencia: El tipo CGP-1-100/BUC podría estar restringido a mantenimiento en algunas regiones o para nuevas instalaciones. Consulte las regulaciones locales o las guías de Iberdrola para confirmar.)*\n")
        ...
        # Para encontrar el rango de corriente aplicable en Tabla 2, necesitamos la Intensidad de la línea,
        # que no está directamente en la Tabla 10 (main ufd_table).
        # Aquí, se hace una búsqueda por la corriente calculada.
        uf_nominal_current_for_lookup = calculated_current 
        uf_cgp_type_val, uf_fuse_capacity_val, uf_cgp_source = get_uf_cgp_type_and_fuse(uf_nominal_current_for_lookup)

        if uf_cgp_type_val != "N/A (Consultar UF Dist.)":
            ...
            st.info("*(Nota: Las capacidades de Fusibles e Interruptores para Unión Fenosa se basan en la Tabla 2 de ES.0100.ES.RE.EIC, vinculada a la corriente nominal de la instalación.)*")
        else:
            st.write("- **Tipo de CGP (Caja General de Protección):** Información no disponible directamente en la tabla de esta compañía para esta corriente.")
            ...
            st.info("*(Nota: Las capacidades de Fusibles/Interruptores no están directamente en la tabla proporcionada por esta compañía para esta corriente; se utiliza una regla de dimensionamiento general como marcador de posición.)*")

        # Unión Fenosa no tiene una regla IGM como Endesa/Iberdrola en el extracto directo, pero sí tiene una IGA en Pág. 57, Tabla 16
...
else: # Si no se encuentran datos específicos de la compañía o power_kw es 0 y current > 0
    st.warning(f"No se encontraron datos específicos de la compañía para una potencia contratada de {power_kw} kW. Se utilizará la Corriente de Diseño ({calculated_current:.2f} A) con recomendaciones genéricas. Por favor, verifique la potencia de entrada o consulte las tablas oficiales.")
    ...
    st.write(f"- **Capacidad de Fusible Recomendada (Mín.):** Aprox. {calculated_current * 1.25:.2f} A (Regla general de seguridad)")
    st.write(f"- **Capacidad de Interruptor Recomendada (Mín.):** Aprox. {calculated_current * 1.25:.2f} A (Regla general de seguridad)")
    st.write("- **Tipo de CGP (Caja General de Protección):** N/A (Consulte al distribuidor)")
    st.write("- **Interruptor General de Maniobra (IGM) Capacity:** N/A (Consulte al distribuidor)")
